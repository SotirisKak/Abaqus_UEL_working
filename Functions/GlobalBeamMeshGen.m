function [GlobalBeamMesh] = GlobalBeamMeshGen(BeamAxisData, BeamMesh, crosslinks, dist_tol)
...
%         end
        ...
        if BeamMesh(i).Order==2 && ~strcmp(BeamDataLoc{1, 8},'straight')
            Xs = [Node1,Node2,Node3]';
            fd = (dN*Xs);
            fdd = (dN2*Xs);
             if sum(fdd)~=0 % There's a chance the segment is straight
