...
max_X = max(Nodes(:,1)); min_X = min(Nodes(:,1));
max_Y = max(Nodes(:,2)); min_Y = min(Nodes(:,2));
max_Z = max(Nodes(:,3)); min_Z = min(Nodes(:,3));
...
fprintf(fid, '\t%d,%f,%f,%f\n',[ntotal+1,max_X-0.5*W,max_Y+0.2*D,max_Z-0.5*H]);
fprintf(fid, '\t%d,%f,%f,%f\n',[ntotal+2,max_X-0.5*W,max_Y-0.5*D,max_Z+0.2*H]);
fprintf(fid, '\t%d,%f,%f,%f\n',[ntotal+3,max_X+0.2*W,max_Y-0.5*D,max_Z-0.5*H]);
fprintf(fid, '\t%d,%f,%f,%f\n',[ntotal+4,max_X+0.4*W,max_Y-0.5*D,max_Z-0.5*H]);
fprintf(fid, '\t%d,%f,%f,%f\n',[ntotal+5,max_X+0.6*W,max_Y-0.5*D,max_Z-0.5*H]);
fprintf(fid, '\t%d,%f,%f,%f\n',[ntotal+6,max_X+0.8*W,max_Y-0.5*D,max_Z-0.5*H]);
...
%     writeCONSTRAINT_UE(fid,'Top_BC', 'Bot_BC', [4,1,6], 1, -1, [-1,-1,-1],ntotal)
%     writeCONSTRAINT_UE(fid,'Left_BC', 'Right_BC', [5,6,2], 1, -1, [-1,-1,-1],ntotal)
%     writeCONSTRAINT_UE(fid,'Front_BC', 'Back_BC', [3,4,5], 1, -1, [-1,-1,-1],ntotal)

    writeCONSTRAINT_UE(fid,'TopS', 'BotS', [4,1,6], 1, -1, [-1,-1,-1],ntotal)
    writeCONSTRAINT_UE(fid,'LeftS', 'RightS', [5,6,2], 1, -1, [-1,-1,-1],ntotal)
    writeCONSTRAINT_UE(fid,'FrontS', 'BackS', [3,4,5], 1, -1, [-1,-1,-1],ntotal)
    
    writeCONSTRAINT_UE(fid,'FTedge', 'BTedge', [3,4,5], 1, -1, [-1,-1,-1],ntotal)
    writeCONSTRAINT_UE(fid,'BTedge', 'BBedge', [4,1,6], 1, -1, [-1,-1,-1],ntotal)
    writeCONSTRAINT_UE(fid,'BBedge', 'FBedge', [3,4,5], 1, -1, [1,1,1],ntotal)
    
    writeCONSTRAINT_UE(fid,'FLedge', 'BLedge', [3,4,5], 1, -1, [-1,-1,-1],ntotal)
    writeCONSTRAINT_UE(fid,'BLedge', 'BRedge', [5,6,2], 1, -1, [-1,-1,-1],ntotal)
    writeCONSTRAINT_UE(fid,'BRedge', 'FRedge', [3,4,5], 1, -1, [1,1,1],ntotal)
    
    writeCONSTRAINT_UE(fid,'LTedge', 'LBedge', [4,1,6], 1, -1, [-1,-1,-1],ntotal)
    writeCONSTRAINT_UE(fid,'LBedge', 'RBedge', [5,6,2], 1, -1, [-1,-1,-1],ntotal)
    writeCONSTRAINT_UE(fid,'RBedge', 'RTedge', [4,1,6], 1, -1, [1,1,1],ntotal)
    
    writeCONSTRAINT_UE(fid,'C6', 'C2', [4,1,6], 1, -1, [1,1,1],ntotal)
    writeCONSTRAINT_UE(fid,'C2', 'C3', [5,6,2], 1, -1, [-1,-1,-1],ntotal)
    writeCONSTRAINT_UE(fid,'C3', 'C4', [3,4,5], 1, -1, [1,1,1],ntotal)
    writeCONSTRAINT_UE(fid,'C4', 'C8', [4,1,6], 1, -1, [-1,-1,-1],ntotal)
    writeCONSTRAINT_UE(fid,'C8', 'C5', [5,6,2], 1, -1, [1,1,1],ntotal)
    writeCONSTRAINT_UE(fid,'C5', 'C1', [4,1,6], 1, -1, [1,1,1],ntotal)
    
    writeCONSTRAINT_SS(fid,'C1', 'C7', [3,1,2],[4,4,5],[5,6,6], -1,-1,-1,ntotal)
...
%% FUNCTIONS
    ...
    function []=writeCONSTRAINT_UE(fid3,NSET_a, NSET_b, RP_i,A_coef,B_coef,C_coef,ntotal_sol)
        
        for ij = 1:3
            fprintf(fid3, '*Equation\n');
            if C_coef(ij)~=0
                fprintf(fid3, '3\n');
            else
                fprintf(fid3, '2\n');
            ...
            fprintf(fid3, [NSET_a,',%d,%d\n'],[ij,A_coef]);
            fprintf(fid3, [NSET_b,',%d,%d\n'],[ij,B_coef]);
            if C_coef(ij)~=0
                if length(RP_i)==1
    ...
    function []=writeCONSTRAINT_SS(fid3,NSET_a, NSET_b, RP_i,RP_j,RP_k,i_coef,j_coef,k_coef,ntotal_sol)
